function Header({setTheme, theme}) {
  ...
  <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} >Dark Mode</button>
        ...
        <input type="search" id="default-search" className="w-full md:w-2/5  block p-4 pl-20 text-sm dark:placeholder-white dark:bg-slate-700 text-gray-700 bg-gray-50 rounded-lg shadow  focus:outline-none " placeholder="Search for a country..." required />
